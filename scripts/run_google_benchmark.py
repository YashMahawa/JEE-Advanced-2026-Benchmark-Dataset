...
def generate(api_key: str, model: str, row: dict, image_path: Path, timeout: int, temperature: float) -> dict:
        ...
        while True:
            with self.lock:
                ...
                if len(self.starts) < self.max_starts:
                    ...
                    return
                sleep_for = self.window_seconds - (now - self.starts[0]) + 0.1
...
def main():
    ...
    def worker(row: dict) -> dict:
        ...
        try:
            return generate(api_key, args.model, row, Path(row["image_path"]), args.timeout, args.temperature)
